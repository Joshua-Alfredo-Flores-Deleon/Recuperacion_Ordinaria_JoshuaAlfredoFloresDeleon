...
function administracion() {
  ...
  const handleSave = async (data) => {
    try {
      if (editEstudiante) {
        await axios.put(
          `${API_URI}/U5EBDb/Estudiantes/${editEstudiante._id}, data`,
        );
      } else {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API_URI}/U5EBDb/Estudiantes`);
        fetchEstudiantes
    } catch {
        console.error('Error deleting Estudiantre: ', err)
